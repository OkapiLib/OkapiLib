...
DOWNLOAD_SQUIGGLES:=$(shell mkdir -p cmake-build-debug && cd cmake-build-debug && cmake -DCMAKE_BUILD_TYPE=Debug -G "CodeBlocks - Unix Makefiles" .. && rm -r ../src/squiggles && mkdir -p ../src/squiggles && find ./squiggles-src/main/src -type f -name '*.cpp' ! -name 'main.cpp' | xargs cp -t ../src/squiggles && rm -r ../include/okapi/squiggles && cp -r ./squiggles-src/main/include ../include/okapi/squiggles && cd ..)
